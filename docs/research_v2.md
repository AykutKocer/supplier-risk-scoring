...
| ESG/CBAM compliance risk flag for EU-facing exporters | Directly tied to a dated 2026 regulatory requirement — strong "real-world relevance" story | Medium-Large | ✅ Done (`compute_cbam_compliance_risk`, `config/cbam_sectors.yaml`, 7 tests) — boolean applicability + gap flag, not blended into either score |
...
| Notebook / README / Power BI dashboard updated for all of the above | The new fields and scores aren't visible anywhere outside `reports/supplier_risk_scores.csv` yet | Medium | ✅ Done — `.pbix` Overview page refreshed (fixed a hardcoded `Columns=N` CSV source gotcha), new Average Financial Risk Score / CBAM Exposed Count cards, Financial Risk Level donut added |
